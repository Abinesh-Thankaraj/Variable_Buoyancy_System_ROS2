...
def generate_launch_description():
    ...
    with sl.group(ns=sl.arg('namespace')):
        ...
        sl.node('auv_control', 'cascaded_pid', parameters=[sl.find('vbs_control', 'cascaded_pid.yaml')],
                output='screen')
        ...
        with sl.group(if_arg='sliders'):
            ...
            sl.node('slider_publisher', 'slider_publisher', name='tilt_control',
                    arguments=[sl.find('vbs_control', 'tilt.yaml')])

    with sl.group(if_arg='rviz'):
        sl.include('vbs_control','rviz_launch.py',
